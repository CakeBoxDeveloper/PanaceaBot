...
def get_db():
    global _fb_app, _db
    if _db is None:
        # Ищем serviceAccountKey.json рядом со скриптом
        key_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'serviceAccountKey.json')
        if not os.path.exists(key_path):
            raise FileNotFoundError(f"serviceAccountKey.json не найден: {key_path}")
...
def gift_premium(email: str, days: int = 30) -> str:
    """Записывает подарок в коллекцию gifts/{email_key}"""
    try:
        db = get_db()
        ...
        db.collection('gifts').document(key).set({
            ...
            'txid': None,
            'method': 'admin_gift',
            'purchasedAt': int(datetime.now(timezone.utc).timestamp() * 1000),
            'durationDays': days,
            'claimed': False,
            'updatedAt': firestore.SERVER_TIMESTAMP,
        })
        return f"✓ Подарок записан для {email} на {days} дней"
    except Exception as e:
        return f"✗ Ошибка: {e}"

def revoke_premium_by_email(email: str) -> str:
    """Помечает подарок как использованный (отзыв до активации)"""
    try:
        db = get_db()
        key = re.sub(r'[^a-z0-9]', '_', email.lower())
        ref = db.collection('gifts').document(key)
        doc = ref.get()
        if not doc.exists:
            return f"✗ Подарок для {email} не найден"
        ref.update({'claimed': True, 'revokedByAdmin': True})
        return f"✓ Подарок для {email} аннулирован"
    except Exception as e:
        return f"✗ Ошибка: {e}"
...
# ─── Запрос email для подарка ─────────────────────────────────────────────────
def ask_gift_email(chat_id: int):
    text = (
        "🎁 <b>Подарить Premium</b>\n\n"
        "Введи email получателя:\n"
        "<i>Подписка активируется при первом входе пользователя</i>"
    )
    user_states[chat_id] = {"state": "waiting_gift_email"}
    send_message(chat_id, text)
...
def handle_message(message: dict):
    ...
    # Ожидаем email для подарка
    elif state == "waiting_gift_email":
        email_pattern = r'^[^\s@]+@[^\s@]+\.[^\s@]+$'
        if re.match(email_pattern, text):
            send_message(chat_id, f"⏳ Записываю подарок для <b>{text}</b>...")
            result = gift_premium(text, days=30)
            send_message(chat_id, result)
            show_admin_panel(chat_id)
        else:
            send_message(chat_id, "❌ Это не похоже на email. Попробуй ещё раз:")
